...
We have built a fully functional MVP. Our Next.js front-end orchestrates a FastAPI Python backend. If an engineer provides a staging URL, our backend spins up headless Chromium instances, injects a defined persona context (like Gen-Z Consumer), scrapes the live DOM using BeautifulSoup, streams "thoughts" via WebSockets powered by Gemini 2.5 Flash, and calculates friction metrics like Rage-Clicks based on element interactivity. We have a working lead magnet at `tryphantom.dev/roast`. We are currently acquiring our first 10 beta testers.
